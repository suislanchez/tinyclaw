...
// in `~/.tinyclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.tinyclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(tinyclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: tinyclaw_dir.join(".secret_key"),
            enabled,
        }
    }
